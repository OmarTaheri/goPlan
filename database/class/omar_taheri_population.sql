...
TRUNCATE TABLE student_plan_semesters;
...
-- ==========================================================
-- Patch triggers to avoid 1442 errors on student_plan_drafts
-- ==========================================================
DROP TRIGGER IF EXISTS trg_before_plan_draft_insert;
DROP TRIGGER IF EXISTS trg_before_plan_draft_update;
DROP TRIGGER IF EXISTS trg_before_student_program_insert;
DROP TRIGGER IF EXISTS trg_before_student_program_update;
...
-- Safe triggers for student_programs to avoid self-updates
...
(3, 'FAS 0210', 'Strategic Academic Skills', 1, 'Non-degree credit course for academic skills (graduation requirement)', TRUE),
...
(403, 'CSC 2305', 'Computer Organization and Architecture', 3, 'Computer systems', TRUE),
...
-- Concentration Optional 1
-- Minor Course 3

-- Semester 7 (Senior Fall) - Summer Internship
(2, 301, 7, 1), -- INT 4302
-- Concentration Required 3 (if Media Production)
-- Concentration Optional 2
-- Minor Course 4

-- Semester 8 (Senior Spring)
(2, 502, 8, 1), -- SSC 4302 Capstone
(2, 130, 8, 2); -- Civic Engagement
-- Concentration Optional 3
-- Minor Course 5
-- Electives
...
-- Clean any auto-created profiles from triggers to avoid duplicates
DELETE FROM student_profiles WHERE user_id = 23;
...
-- Reset drafts for Amina and recreate default (avoid hard-coded IDs)
DELETE FROM student_plan_drafts WHERE student_id = 23;
INSERT INTO student_plan_drafts (student_id, name, is_default) VALUES
(23, 'Default Plan', TRUE);   -- Amina
...
-- Ensure Amina has a draft before adding semesters (handles partial runs)
INSERT INTO student_plan_drafts (student_id, name, is_default)
SELECT 23, 'Default Plan', TRUE
WHERE NOT EXISTS (SELECT 1 FROM student_plan_drafts WHERE student_id = 23);

-- Clear semesters tied to Amina's default draft (avoid FK issues from hard-coded IDs)
DELETE FROM student_plan_semesters
WHERE draft_id IN (
  SELECT draft_id
  FROM student_plan_drafts
  WHERE student_id = 23 AND is_default = TRUE
);

-- Re-insert the semester using the actual draft id
INSERT INTO student_plan_semesters (draft_id, semester_number, term, year, is_locked)
SELECT d.draft_id, 1, 'SPRING', 2025, FALSE
FROM student_plan_drafts d
WHERE d.student_id = 23 AND d.is_default = TRUE
LIMIT 1;
