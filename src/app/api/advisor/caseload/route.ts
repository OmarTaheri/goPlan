...
/**
 * GET /api/advisor/caseload
 * Get advisor's caseload with pending plan counts (based on semester approvals)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // We count pending SEMESTER approvals, not individual courses
    let sql = `
      SELECT
        u.user_id,
        u.username,
        ...
        minors.minor_name,
        sp.enrollment_year,
        (SELECT COUNT(*) FROM semester_approvals sa
         WHERE sa.student_id = u.user_id AND sa.approval_status = 'PENDING') as pending_plans,
        (SELECT COUNT(*) FROM semester_approvals sa
         WHERE sa.student_id = u.user_id AND sa.approval_status = 'APPROVED') as approved_plans
