...
import { Search, Loader2, X, ClipboardList, BookOpen } from "lucide-react";
...
export default function EditConcentrationPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => { if (!isNaN(programId)) { loadProgram(); loadCourses(); loadMajors(); } }, [programId]);
  ...
  async function loadProgram() {
    try {
      ...
      setBasicInfo({ name: program.name, code: program.code || "", parent_program_id: program.parent_program_id || 0, total_credits_required: program.total_credits_required });
      const reqRes = await fetch(`/api/admin/programs/${programId}/requirements`);
      if (reqRes.ok) {
        const { requirement_groups } = await reqRes.json();
        for (const g of requirement_groups) { g.name === "Required Courses" ? setMandatoryCourses(g.courses || []) : (setOptionalPool(g.courses || []), setOptionalCount(g.min_courses_required || 4)); }
      }
  ...
  async function loadMajors() { try { const res = await fetch("/api/admin/programs?type=MAJOR"); if (res.ok) setMajors((await res.json()).programs || []); } catch { } }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Concentration updated"); router.push("/dashboard/admin/programs");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold">Edit Concentration: {basicInfo.name}</h1><p className="text-muted-foreground">Update settings and courses</p></div>
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5 text-primary" />Courses</CardTitle></CardHeader><CardContent><div className="grid grid-cols-3 gap-4">
        <div><Label>Search</Label><Input placeholder="Search..." value={courseSearch} onChange={e => setCourseSearch(e.target.value)} className="mb-2" /><ScrollArea className="h-60 border rounded">{filteredCourses.slice(0, 30).map(c => <div key={c.course_id} className="p-2 border-b flex justify-between"><span>{c.course_code}</span><div className="flex gap-1"><Button size="sm" variant="outline" className="h-6 text-xs" onClick={() => addToMandatory(c)}>Required</Button><Button size="sm" variant="outline" className="h-6 text-xs" onClick={() => addToOptional(c)}>Optional</Button></div></div>)}</ScrollArea></div>
        <div><Label className="text-green-700">Required ({mandatoryCourses.length} • {mandatoryCredits} SCH)</Label><ScrollArea className="h-60 border-2 border-dashed border-green-300 rounded p-2">{mandatoryCourses.map(c => <div key={c.course_id} className="flex justify-between p-1"><span>{c.course_code}</span><Button size="sm" variant="ghost" onClick={() => setMandatoryCourses(mandatoryCourses.filter(x => x.course_id !== c.course_id))}><X className="h-4 w-4" /></Button></div>)}</ScrollArea></div>
        <div><Label className="text-orange-700">Optional ({optionalPool.length})</Label><div className="flex gap-2 mb-2"><span className="text-sm">Select</span><Input type="number" value={optionalCount} onChange={e => setOptionalCount(parseInt(e.target.value) || 1)} className="w-16 h-8" /></div><ScrollArea className="h-52 border-2 border-dashed border-orange-300 rounded p-2">{optionalPool.map(c => <div key={c.course_id} className="flex justify-between p-1"><span>{c.course_code}</span><Button size="sm" variant="ghost" onClick={() => setOptionalPool(optionalPool.filter(x => x.course_id !== c.course_id))}><X className="h-4 w-4" /></Button></div>)}</ScrollArea></div>
      </div></CardContent></Card>
    </div>
  );
}
